...
async function buildSyncedShort({ sourceVideoPath, segments, workDir, outputPath }) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    try {
      ...
      // Sanity check: make sure the cut actually produced a valid, playable
      // clip before wasting time stretching/muxing a broken file.
      const rawClipDur = await getDurationSeconds(rawClipPath).catch(() => 0);
      if (!rawClipDur || rawClipDur <= 0) {
        throw new Error(`raw clip ${idx} came out empty/invalid (no stream)`);
      }
      ...
      console.warn(`    !! segment ${idx} failed, skipping it and continuing: ${err.message}`);
    }
  }

  if (segmentOutputs.length === 0) {
    throw new Error('buildSyncedShort: every segment failed, nothing to concat');
  }
